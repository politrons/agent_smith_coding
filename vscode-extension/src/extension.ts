...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("agentSmithCoding.openChat", () =>
      vscode.commands.executeCommand("workbench.view.extension.agentSmithCoding")
    ),
    vscode.commands.registerCommand("agentSmithCoding.clearChat", () => provider.clear()),
    vscode.commands.registerCommand("agentSmithCoding.stop", () => provider.stopCurrentRun()),
    vscode.commands.registerCommand("agentSmithCoding.askBrowser", () => provider.askFromInput("browser")),
    vscode.commands.registerCommand("agentSmithCoding.askWorkflow", () => provider.askFromInput("coding_workflow")),
    vscode.commands.registerCommand("agentSmithCoding.askCoder", () => provider.askFromInput("coder")),
    vscode.commands.registerCommand("agentSmithCoding.askFilesystem", () => provider.askFromInput("filesystem")),
    vscode.commands.registerCommand("agentSmithCoding.askMemory", () => provider.askFromInput("memory")),
    vscode.commands.registerCommand("agentSmithCoding.askPlanner", () => provider.askFromInput("planner")),
    vscode.commands.registerCommand("agentSmithCoding.askTerminal", () => provider.askFromInput("terminal")),
    vscode.workspace.onDidChangeWorkspaceFolders(() => provider.refreshWorkspace()),
    output
  );
}
...
class ChatViewProvider implements vscode.WebviewViewProvider {
  static readonly viewType = "agentSmithCoding.chat";
  private static readonly storageKey = "agentSmithCoding.chat.messages";
  ...
  private ollamaModelsCache?: { checkedAt: number; models: string[]; error?: string };
  ...
  async askFromInput(agent: AgentName): Promise<void> {
  ...
  private async handleWebviewMessage(message: { type: string; codingModel?: string; browserModel?: string; model?: string; prompt?: string }): Promise<void> {
  ...
  private async runAgent(agent: AgentName, prompt: string, requestedCodingModel?: string, requestedBrowserModel?: string): Promise<void> {
    if (this.busy) {
    ...
    const logPoller = setInterval(() => {
      ...
      terminalLogTail = this.pollTerminalLog(
        terminalLogTail,
        settings,
        assistantIndex,
        () => {
    ...
    const heartbeat = setInterval(() => {
      ...
      this.updateRuntimeStatus(
        assistantIndex,
        `Agent ${phase} working... ${elapsedSeconds}s. ${outputState}.${extra}`
      );
    }, 15000);
    ...
    child.stdout.on("data", (chunk: Buffer) => {
      ...
      if (attemptedToolCall) {
        this.appendRuntimeLog(
          assistantIndex,
          `model: emitted unexecuted MCP tool markup for ${attemptedToolCall}`,
          this.statusForToolCall(attemptedToolCall)
        );
      }
    ...
    child.stderr.on("data", (chunk: Buffer) => {
      ...
      this.appendRuntimeLog(
        assistantIndex,
        this.logLineFromProcessOutput(text),
        this.statusFromStderr(text, settings.codingModel)
      );
    });
    ...
    child.on("close", async (code: number | null) => {
      ...
      try {
        if (this.stopRequested) {
          ...
          this.appendRuntimeLog(
            assistantIndex,
            "runtime: unexecuted tool-call markup detected; stopped after one workflow pass"
          );
          ...
          this.appendRuntimeLog(
            assistantIndex,
            "runtime: provider/internal model error detected; stopped after one workflow pass"
          );
          ...
          this.appendRuntimeLog(
            assistantIndex,
            "runtime: non-actionable implementation answer detected; stopped after one workflow pass"
          );
        }
      } catch (error) {
      ...
      this.appendRuntimeLog(
        assistantIndex,
        `runtime: finished with code ${code ?? "unknown"}`
      );
  ...
  stopCurrentRun(): void {
    ...
    if (assistantIndex >= 0) {
      this.appendRuntimeLog(
        assistantIndex,
        "runtime: stop requested by user",
        "Stopping agent..."
      );
    }
    ...
    if (this.currentProcess && this.currentProcess.exitCode === null && this.currentProcess.signalCode === null) {
      ...
      this.appendRuntimeLog(
        assistantIndex,
        "runtime: stop requested while no agent subprocess was active; workflow will stop before next phase"
      );
    }
    this.postState();
  }

  private killProcessTree(child: ChildProcessWithoutNullStreams, signal: NodeJS.Signals): boolean {
    const pid = child.pid;
    if (!pid) {
      return false;
    }
  ...
  private lastPendingAssistantIndex(): number {
  ...
  private async runControlledWorkflow(prompt: string, settings: RunSettings, assistantIndex: number): Promise<void> {
    try {
      const plannerPrompt = this.buildPlannerPhasePrompt(prompt, settings);
      const planner = await this.runAgentPhase(
        "planner",
        "Plan the latest user request once. Produce subtasks, delegation, risks, and validation guidance only.",
        plannerPrompt,
        settings,
        assistantIndex,
        `User prompt: ${prompt}`
      ...
      const coder = await this.runAgentPhase(
        "coder",
        "Implement the planner output once. Inspect files, edit when needed, and finish with one validation command.",
        coderPrompt,
        settings,
        assistantIndex,
      ...
      const terminal = this.shouldSkipTerminalPhase(coder.content)
        ...
        : await this.runAgentPhase(
          "terminal",
          "Run the single validation command from coder once and report command, cwd, exit code, timeout, stdout, and stderr.",
          this.buildTerminalPhasePrompt(prompt, settings, plannerForWorkflow.content, coder.content),
          settings,
          assistantIndex,
      ...
      if (this.shouldSkipTerminalPhase(coder.content)) {
        this.appendRuntimeLog(
          assistantIndex,
          "agent terminal skipped: coder reported validation command unavailable",
          "Agent terminal skipped."
        );
      }
      ...
      this.appendRuntimeLog(
        assistantIndex,
        "workflow: planner, coder, and terminal completed one controlled pass",
        "Workflow completed."
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private runAgentPhase(
    phaseAgent: AgentName,
    ...
    this.appendRuntimeLog(
      assistantIndex,
      `agent ${phaseAgent} task input: ${taskDescription} Input summary: ${this.summarizeForLog(logInput ?? phasePrompt, 1200)}`,
      `Agent ${this.humanAgentName(phaseAgent)} working...`
    );
    ...
    return new Promise((resolve) => {
      ...
      let sawStdout = false;
      let sawFastAgentEvent = false;
      const phaseTerminalEvents: string[] = [];
      const startedAt = Date.now();
      let lastProcessOutputAt = startedAt;
      let logTail = this.createFastAgentLogTail(settings);
      let terminalLogTail = this.createTerminalLogTail(settings);
      ...
      const heartbeat = setInterval(() => {
        ...
        this.updateRuntimeStatus(
          assistantIndex,
          `Agent ${this.humanAgentName(phaseAgent)} working... ${elapsedSeconds}s. ${outputState}.${extra}`
        );
      }, 15000);

      const finish = (result: AgentPhaseResult): void => {
        if (settled) {
          return;
        }
        settled = true;
        ...
        this.appendRuntimeLog(
          ...
          result.failed
            ...
            : `agent ${phaseAgent} output: ${this.summarizeForLog(result.content, 1200)}`,
          result.failed ? `Agent ${this.humanAgentName(phaseAgent)} failed.` : `Agent ${this.humanAgentName(phaseAgent)} finished.`
        );
        resolve(result);
      };
      ...
      child.stdout.on("data", (chunk: Buffer) => {
        ...
        if (attemptedToolCall) {
          this.appendRuntimeLog(
            assistantIndex,
            `agent ${phaseAgent}: emitted unexecuted MCP tool markup for ${attemptedToolCall}`,
            this.statusForToolCall(attemptedToolCall)
          );
        }
      });
      ...
      child.stderr.on("data", (chunk: Buffer) => {
        ...
        this.appendRuntimeLog(
          assistantIndex,
          this.logLineFromProcessOutput(text),
          this.statusFromStderr(text, this.modelForAgent(phaseAgent, settings))
        );
      });
  ...
  private async evaluateAndAppendRun(
    prompt: string,
    settings: RunSettings,
    assistantIndex: number,
    planner: AgentPhaseResult,
    coder?: AgentPhaseResult,
    terminal?: AgentPhaseResult
  ): Promise<AgentPhaseResult> {
  ...
  private runEvaluatorPhase(
    ...
    this.appendRuntimeLog(
      assistantIndex,
      "agent evaluator task input: validate planner, coder, terminal, and MCP activity outputs",
      "Agent evaluator working..."
    );
    ...
    return new Promise((resolve) => {
      ...
      const child = spawn(
        settings.pythonPath,
        ["-m", "open_agent_coding.agent_run_evaluator", "--stdin"],
        {
          cwd: settings.projectPath,
          detached: true,
          env: {
      ...
      const finish = (result: AgentPhaseResult): void => {
        ...
        this.appendRuntimeLog(
          ...
          result.failed
            ...
            : `agent evaluator output: ${this.summarizeForLog(result.content, 1200)}`,
          result.failed ? "Agent evaluator failed." : "Agent evaluator finished."
        );
        resolve(result);
      };
  ...
  private emptyPhaseResult(agent: AgentName): AgentPhaseResult {
    return {
      agent,
      content: "",
      stderr: "",
  ...
  private parseEvaluatorOutput(stdout: string): { passed?: boolean; markdown?: string; summary?: string } {
    try {
  ...
  private stopControlledWorkflow(assistantIndex: number, phaseAgent: AgentName, result: AgentPhaseResult): void {
    this.messages[assistantIndex].content = [
      this.messages[assistantIndex].content.trim(),
    ...
    this.appendRuntimeLog(
      assistantIndex,
      `workflow: stopped after ${phaseAgent} - ${result.failureReason ?? "unknown failure"}`,
      `Workflow stopped after ${this.humanAgentName(phaseAgent)}.`
    );
  }

  private appendPhaseOutputToContent(assistantIndex: number, title: string, result: AgentPhaseResult): void {
  ...
  private phaseFailureReason(code: number | null, content: string, stderr: string, phaseAgent: AgentName): string | undefined {
    if (this.stopRequested) {
  ...
  private shouldSkipTerminalPhase(coderOutput: string): boolean {
    return /Validation command\s*:\s*unavailable/i.test(coderOutput);
  }

  private buildSkippedTerminalResult(coderOutput: string): AgentPhaseResult {
  ...
  private shouldUsePlannerFallback(result: AgentPhaseResult): boolean {
  ...
  private buildPlannerFallbackOutput(prompt: string, settings: RunSettings, reason?: string): string {
  ...
  private appendPlannerFallbackLogs(assistantIndex: number, prompt: string, settings: RunSettings, reason?: string): void {
    const implementation = this.isImplementationPrompt(prompt);
    const needsBrowser = this.promptLikelyNeedsBrowser(prompt);
    this.appendRuntimeLog(
      assistantIndex,
      `agent planner fallback: planner produced no final text (${reason ?? "unknown reason"}); generated visible single-pass delegation`,
      "Agent planner fallback generated."
    );
    this.appendRuntimeLog(
      assistantIndex,
      [
    ...
    this.appendRuntimeLog(
      ...
      implementation
        ...
        : "agent coder task input: inspect files, read README/project markers, answer latest question, skip edits, return read-only validation note",
      "Agent coder queued."
    );
    this.appendRuntimeLog(
      assistantIndex,
      implementation
        ...
        : "agent terminal task input: skip command unless coder provides a safe read-only validation command",
      "Agent terminal queued."
    );
  }

  private isNonActionableAnswer(content: string): boolean {
  ...
  private terminalOutputLooksFailed(content: string): boolean {
  ...
  private modelForAgent(agent: AgentName, settings: RunSettings): string {
  ...
  private buildCoderPhasePrompt(prompt: string, settings: RunSettings, plannerOutput: string): string {
    return [
  ...
  private buildPlannerPhasePrompt(prompt: string, settings: RunSettings): string {
    return [
  ...
  private buildTerminalPhasePrompt(prompt: string, settings: RunSettings, plannerOutput: string, coderOutput: string): string {
    return [
  ...
  private summarizeForLog(text: string, maxChars: number): string {
  ...
  private clipForChat(text: string, maxChars: number): string {
  ...
  private addMessage(role: ChatRole, content: string, agent?: AgentName): void {
    this.messages.push({ role, content, agent });
    this.postState();
  }

  private appendRuntimeLog(index: number, line: string, status?: string): void {
    const message = this.messages[index];
    if (!message) {
      return;
    }
  ...
  private updateRuntimeStatus(index: number, status: string): void {
    const message = this.messages[index];
  ...
  private createFastAgentLogTail(settings: RunSettings): FastAgentLogTail {
    const logPath = this.fastAgentLogPath(settings);
    try {
      return { offset: existsSync(logPath) ? statSync(logPath).size : 0, carry: "" };
    } catch {
      return { offset: 0, carry: "" };
    }
  }

  private pollFastAgentLog(
    tail: FastAgentLogTail,
    settings: RunSettings,
    assistantIndex: number,
    onEvent?: () => void
  ): FastAgentLogTail {
  ...
  private fastAgentLogPath(settings: RunSettings): string {
    return join(settings.projectPath, "fastagent.jsonl");
  }

  private createTerminalLogTail(settings: RunSettings): FastAgentLogTail {
  ...
  private pollTerminalLog(
    tail: FastAgentLogTail,
    settings: RunSettings,
    assistantIndex: number,
    onEvent?: () => void,
    onFormattedEvent?: (event: { line: string; status?: string }) => void
  ): FastAgentLogTail {
    const logPath = this.terminalLogPath(settings);
    if (!existsSync(logPath)) {
      return tail;
  ...
  private terminalLogPath(settings: RunSettings): string {
    return settings.memoryFilePath.replace(/\.jsonl$/i, ".terminal.jsonl");
  }

  private formatTerminalLogLine(line: string): { line: string; status?: string } | undefined {
    if (!line.trim()) {
      return undefined;
    }
  ...
  private terminalCommandText(value: unknown): string {
  ...
  private terminalOutputSummary(value: unknown): string {
  ...
  private formatFastAgentLogLine(line: string): { line: string; status?: string } | undefined {
    if (!line.trim()) {
      return undefined;
    }
  ...
  private agentFromNamespace(value: unknown): string {
  ...
  private humanAgentName(value: unknown): string {
    const raw = typeof value === "string" ? value : "";
    const normalized = raw.replace(/^filesystem$/, "file system").replace(/_/g, " ").trim();
    return normalized;
  }

  private humanToolName(value: unknown): string {
    const raw = typeof value === "string" ? value : "";
    return raw
  ...
  private statusForFastAgentEvent(agentName: unknown, serverName: unknown, toolName: unknown): string {
  ...
  private loadMessages(): ChatMessage[] {
    const stored =
      this.loadMessagesFromStorageFile() ??
      this.context.workspaceState.get<unknown>(ChatViewProvider.storageKey);
  ...
  private saveMessages(): void {
    ...
    void this.context.workspaceState.update(
      ChatViewProvider.storageKey,
      history
    );
    this.saveMessagesToStorageFile(history);
  }

  private loadMessagesFromStorageFile(): unknown {
  ...
  private saveMessagesToStorageFile(messages: ChatMessage[]): void {
    const path = this.storageFilePath();
    if (!path) {
      return;
    }
  ...
  private storageFilePath(): string | undefined {
    ...
    return root ? join(root.fsPath, "chat-history.json") : undefined;
  }

  private defaultAgent(): AgentName {
    const configured = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<AgentName>("defaultAgent");
  ...
  private readSettings(requestedCodingModel?: string, requestedBrowserModel?: string): RunSettings {
    ...
    const codingModel = this.normalizeModel(requestedCodingModel || config.get<string>("coderModel") || this.defaultCodingModel());
    const browserModel = this.normalizeModel(requestedBrowserModel || config.get<string>("browserModel") || this.defaultBrowserModel());
  ...
  private ensureMemoryDirectory(settings: RunSettings): void {
    try {
      mkdirSync(dirname(settings.memoryFilePath), { recursive: true });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private buildWorkspaceAwarePrompt(prompt: string, settings: RunSettings, includeSnapshot = true): string {
    const sections = [
    ...
    if (includeSnapshot) {
      sections.push(
        "VS Code workspace snapshot before MCP execution:",
        "Use this as starting context and verify with Filesystem MCP before editing. It exists specifically to avoid generic answers such as asking what type of project is open.",
        this.buildWorkspaceSnapshot(settings.targetWorkspace, 180, 24, 32_000),
        ""
      );
    }
  ...
  private buildWorkspaceSnapshot(
    workspacePath: string,
    maxFiles = 180,
    maxImportantFiles = 24,
  ...
  private collectWorkspaceFiles(workspacePath: string, maxFiles = 180): string[] {
    const files: string[] = [];
    const visit = (directory: string, depth: number): void => {
      ...
      for (const entry of entries) {
        ...
        try {
          if (statSync(absolutePath).size > 512_000) {
            continue;
          }
        } catch {
          continue;
  ...
  private shouldSkipSnapshotEntry(name: string): boolean {
    return new Set([
      ".git",
      ".hg",
      ".svn",
  ...
  private shouldSkipSnapshotFile(path: string): boolean {
    return /\.(png|jpe?g|gif|webp|ico|icns|pdf|zip|tar|gz|tgz|jar|class|pyc|pyo|so|dylib|dll|vsix)$/i.test(path);
  }

  private isImportantSnapshotFile(path: string): boolean {
  ...
  private memoryFilePathForWorkspace(workspacePath: string): string {
    const root = this.context.globalStorageUri ?? this.context.storageUri;
    const storageRoot = root?.fsPath ?? join(workspacePath, ".agent-smith-coding");
    const normalized = workspacePath.trim() || "default-workspace";
    const hash = createHash("sha256").update(normalized).digest("hex").slice(0, 16);
    const slug = this.slugifyWorkspaceName(basename(normalized) || "workspace");
    return join(storageRoot, "workspace-memory", `${slug}-${hash}.jsonl`);
  }
  ...
  private defaultCodingModel(): string {
    const configured = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<string>("defaultCodingModel");
  ...
  private defaultBrowserModel(): string {
    const configured = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<string>("defaultBrowserModel");
    return this.normalizeModel(configured || this.preferredToolModel());
  }

  private codingModelOptions(): string[] {
    const configured = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<string[]>("codingModelOptions");
    const fallback = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<string[]>("modelOptions");
    const defaults = [
      "qwen3-coder:30b",
      "qwen3.5:latest",
      "gpt-oss:20b",
  ...
  private browserModelOptions(): string[] {
    const configured = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<string[]>("browserModelOptions");
    const fallback = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<string[]>("modelOptions");
    const defaults = [
  ...
  private configuredAgentModel(
    config: vscode.WorkspaceConfiguration,
    key: string,
    fallback: string
  ): string {
    return this.normalizeModel(config.get<string>(key) || fallback);
  }

  private installedOllamaModels(warnings: string[]): string[] {
    const now = Date.now();
    ...
    try {
      const outputText = execFileSync("ollama", ["list"], {
        encoding: "utf8",
        timeout: 2500
      });
  ...
  private preferredToolModel(): string {
    const configured = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<string>("toolModel");
    return this.normalizeModel(configured || "qwen3.5:latest");
  }

  private warnIfOllamaModelMissing(label: string, model: string, installedModels: string[], warnings: string[]): void {
  ...
  private phaseForElapsed(prompt: string, elapsedSeconds: number): string {
  ...
  private isImplementationPrompt(prompt: string): boolean {
  ...
  private shouldRetryNonActionableAnswer(prompt: string, content: string): boolean {
  ...
  private statusForToolCall(toolName: string): string {
  ...
  private statusFromStderr(text: string, model: string): string {
  ...
  private logLineFromProcessOutput(text: string): string {
    const compact = text.replace(/\s+/g, " ").trim();
  ...
  private chatFontFamily(): string {
    const configured = vscode.workspace
      .getConfiguration("agentSmithCoding")
      .get<string>("chatFontFamily");
  ...
  private renderHtml(webview: vscode.Webview): string {
    ...
    const agentIconUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, "resources", "agent.png")
    );
